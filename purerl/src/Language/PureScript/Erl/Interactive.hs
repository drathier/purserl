...
module Language.PureScript.Erl.Interactive
  ( handleCommand
  , module Interactive
...
import           Language.PureScript.Interactive.Module       as Interactive hiding (modulesDir)
...
runMake mk = fst <$> P.runMake (P.defaultOptions { P.optionsCodegenTargets = S.singleton P.CoreFn }) mk

modulesDir :: FilePath
modulesDir = ".purerl_repl/"
...
handleKindOf print' typ = do
  ...
  case e of
    Left errs -> printErrors errs
    Right (_, env') ->
      case Env.getTypeSynonymTypeModu mName (P.ProperName "IT") env' of
        Just typ' -> do

...
instance Show a => Show (Point a) where
  show (Pt pt) =
    case unsafePerformIO (readIORef pt) of
      Info i ref -> "(UFPoint<" <> show (unsafePerformIO (readIORef i)) <> "> " <> show (unsafePerformIO (readIORef ref)) <> ")"
      Link point -> "(UFLink " <> show point <> ")"

...
readCborFileIO path = do
  ...
  -- return (join r)
  -- TODO[drathier]: catch deserialise failure is in the wrong place here, but types are tricky to solve, so leaving for now
  mcontents <- catchDoesNotExist $ catchDeserialiseFailure path $ BSL.readFile path
  case join mcontents of
    Nothing -> pure Nothing
    Just contents ->
      pure (Just (Serialise.deserialise (Zip.decompress contents)))
...
writeCborFileIO path value = do
  ...
  BSL.writeFile path (Zip.compressWith (Zip.defaultCompressParams {Zip.compressLevel = Zip.bestSpeed}) contents)

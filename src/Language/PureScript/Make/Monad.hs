...
readCborFile :: (MonadIO m, MonadError MultipleErrors m) => Binary a => FilePath -> m (Maybe a)
readCborFile path =
...
readCborFileIO path = do
  r <- catchDoesNotExist $ either (const Nothing) Just <$> Binary.decodeFileOrFail path
  return (join r)
...
writeCborFile :: (MonadIO m, MonadError MultipleErrors m) => Binary a => FilePath -> a -> m ()
